...
using HQF.Daily.Migrations.SeedData;
...
namespace HQF.Daily.Tests
{
    public abstract class DailyTestBase : AbpIntegratedTestBase<DailyTestModule>
    {
        ...
        protected void UsingDbContext(Action<DailyDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DailyDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DailyDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DailyDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DailyDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DailyDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DailyDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DailyDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DailyDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DailyDbContext, Task<T>> func)
        {
            T result;
